 ...
 * 
 * 
 * 这一个完全就是老师那个QueryPanel复制过来的
 */
public class UpdatePanel {
	private JPanel panel;
	private JTable allStudentTable, allCourseTable, allUserTable;
	private JTextField editStudentIdField, editSourceField, editNameField, editRealNameField, dobField, editPhoneField,
			editAddressField, editPasswordField;
	...
	QueryPanel updateQuery;

	public UpdatePanel(StudentManagerView view, int role, String name, String password) {
		this.parentView = view;
		// 创建控制器，并关联视图
		StudentController teacherController = new StudentController(view);
		// 加载学生数据并显示
		teacherController.loadStudentData();
		initialize(1, name, password);
	}
	...
	private void initialize(int role, String name, String password) {
		...
		allCourseTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (selectedRow != -1) {
					// editStudentIdField.setText(allCourseTable.getValueAt(selectedRow,
					// 0).toString());
					// editRealNameField.setText(allCourseTable.getValueAt(selectedRow,
					// 1).toString());
					...
					double source = courseController.showCurseSource(Integer.parseInt(studentId),
							Integer.parseInt(allCourseTable.getValueAt(selectedRow, 0).toString()));
		...
		if (role == 0) {
			...
			JPanel editPanel = createSearchPanel(studentInfo.get(0)[0], this.editStudentName, this.editStudentPassword,
					"95.2", studentInfo.get(0)[2], studentInfo.get(0)[3], studentInfo.get(0)[4]);
